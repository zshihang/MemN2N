...
class MemN2N(nn.Module):
    ...
    def __init__(self, params):
        ...
        if self.use_lw:
            ...
            self.out = nn.Parameter(
                I.normal(torch.FloatTensor(self.input_size, self.embed_size), 0, 0.1))
        ...
        # temporal matrix
        self.TA = nn.Parameter(
            I.normal(torch.FloatTensor(self.memory_size, self.embed_size), 0, 0.1))
        self.TC = nn.Parameter(
            I.normal(torch.FloatTensor(self.memory_size, self.embed_size), 0, 0.1))
    ...
    def forward(self, story, query):
        ...
        for i in range(self.num_hops):
            ...
            probability = F.softmax((memory @ state.unsqueeze(-1)).squeeze(), dim=-1)
            response = (probability.unsqueeze(1) @ output).squeeze()
            if self.use_lw:
    ...
    def compute_weights(self, J):
        d = self.embed_size
        if self.use_bow:
            weights = Variable(torch.ones(J, d))
        else:
            func = lambda j, k: 1 - (j + 1)/J - (k + 1)/d*(1 - 2*(j + 1)/J)    # 0-based indexing
            weights = Variable(torch.from_numpy(np.fromfunction(func, (J, d), dtype=np.float32)))
        return weights
